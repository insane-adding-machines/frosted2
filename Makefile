...
%.o : src/%.rs
	rustc -g --target=thumbv7m-none-eabi -O -Z no-landing-pads $^ --emit obj -L libcore -o $@

kernel.elf: $(KERNEL_OBJ)
	arm-none-eabi-gcc -g -ggdb $^ -Wl,--gc-sections -T$(PORT).ld -o $@ -Wl,-Map,kernel.map -nostdlib
...
clean:
	rm -f image.bin kernel.elf kernel.map $(KERNEL_OBJ) 
